...
class BrowserAgent(BaseBrowserAgent):
    ...
    def __init__(self, execution_mode='text', enable_gif=True, case_name=None, user=None):
        ...
        super().__init__(execution_mode='text', enable_gif=enable_gif, case_name=case_name, user=user)
...
def run_ai_task_sync(task_description: str, planned_tasks=None, callback=None, should_stop=None, execution_mode='text', user=None):
    agent = BrowserAgent(execution_mode='text', user=user)
    return asyncio.run(agent.run_task(task_description, planned_tasks, callback, should_stop))

def analyze_task_sync(task_description: str, execution_mode='text', user=None):
    agent = BrowserAgent(execution_mode='text', user=user)
    return asyncio.run(agent.analyze_task(task_description))

def run_full_process_sync(task_description: str, analysis_callback=None, step_callback=None, should_stop=None, execution_mode='text', enable_gif=True, case_name=None, user=None):
    logger.info(f"DEBUG: Entering run_full_process_sync with execution_mode=text, enable_gif={enable_gif}")

    agent = BrowserAgent(execution_mode='text', enable_gif=enable_gif, case_name=case_name, user=user)
