...
export default {
  ...
  methods: {
    ...
    async exportToXMindFile() {
      ...
      try {
        const dateStr = new Date().toISOString().slice(0, 10)
        const fileName = this.$t('taskDetail.xmindFileName', { taskId: this.taskId, date: dateStr })
        const title = this.$t('taskDetail.xmindTitle', { taskId: this.taskId })
        ...
        ElMessage.success(this.$t('taskDetail.xmindExportSuccess'))
      } catch (error) {
        console.error('Export XMind failed:', error)
        ElMessage.error(this.$t('taskDetail.xmindExportFailed') + ': ' + (error.message || ''))
      } finally {
        this.isExporting = false
      }
    }
...
.export-btn.xmind-btn {
  background: #e67e22;
}

.export-btn.xmind-btn:hover:not(:disabled) {
  background: #d35400;
}
