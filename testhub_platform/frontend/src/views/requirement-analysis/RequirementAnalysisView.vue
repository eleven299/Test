      ...
      <div v-if="isGenerating || showResults" class="generation-progress">
        <div class="progress-card">
          ...
          <div v-if="showResults" class="completion-actions">
            ...
            <button class="download-btn xmind-btn" @click="downloadTestCasesAsXMind">
              <span>🧠 {{ $t('requirementAnalysis.downloadXMind') }}</span>
            </button>
...
import { exportToXMind, parseTestCasesFromContent } from '@/utils/xmind'
...
export default {
  ...
  methods: {
    ...
    startStreamingProgress() {
      ...
      // 创建EventSource（通过query参数传递JWT token，不再依赖session cookie）
    ...
    // 导出测试用例为 XMind
    async downloadTestCasesAsXMind() {
      try {
        const finalTestCases = this.generationResult.final_test_cases
        const taskId = this.generationResult.task_id
        ...
        let testCases = []
        if (tableFormat.length > 1) {
          // 表格格式
          const headers = tableFormat[0].map(h => h.toLowerCase())
          for (let i = 1; i < tableFormat.length; i++) {
            const row = tableFormat[i]
            const tc = { caseId: '', scenario: '', precondition: '', steps: '', expected: '', priority: 'P2' }
            headers.forEach((h, idx) => {
              const val = this.convertBrToNewline(row[idx] || '')
              if (h.includes('id') || h.includes('编号')) tc.caseId = val
              else if (h.includes('目标') || h.includes('场景') || h.includes('标题')) tc.scenario = val
              else if (h.includes('前置') || h.includes('前提')) tc.precondition = val
              else if ((h.includes('步骤') || h.includes('操作')) && !h.includes('预期')) tc.steps = val
              else if (h.includes('预期') || h.includes('结果')) tc.expected = val
              else if (h.includes('优先')) tc.priority = val
            })
            if (tc.scenario || tc.caseId) testCases.push(tc)
          }
        } else {
          // 结构化格式
          testCases = parseTestCasesFromContent(filteredContent)
        }

        if (testCases.length === 0) {
          ElMessage.warning(this.$t('requirementAnalysis.noCasesToExport'))
          return
        }

        const dateStr = new Date().toISOString().slice(0, 10)
        const fileName = this.$t('requirementAnalysis.xmindFileName', { taskId: taskId, date: dateStr })
        const title = this.$t('requirementAnalysis.xmindTitle', { taskId: taskId })

        await exportToXMind({ fileName, title, testCases })
        ElMessage.success(this.$t('requirementAnalysis.xmindDownloadSuccess'))
      } catch (error) {
        console.error('XMind export failed:', error)
        ElMessage.error(this.$t('requirementAnalysis.xmindDownloadFailed') + ': ' + (error.message || ''))
      }
    },
...
.completion-actions .download-btn.xmind-btn {
  background: #e67e22;
}

.completion-actions .download-btn.xmind-btn:hover {
  background: #d35400;
